...
cc::utils::Result<cc::models::MealLog> JsonMealRepository::getById(const int id) {
    ...
    if (infile.is_open() && infile.peek() != std::ifstream::traits_type::eof()) {
        ...
        return cc::utils::Result<cc::models::MealLog>::fail(cc::utils::ErrorCode::NotFound,
                                                            "item not found");
    } else {
...
cc::utils::Result<std::vector<cc::models::MealLog>> JsonMealRepository::list(int offset,
                                                                             int limit) {
    std::lock_guard<std::mutex> lock(this->mtx_);
    std::ifstream infile(this->filePath_);
    nlohmann::json file_content;
