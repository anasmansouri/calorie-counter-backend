...
protected:
    ...
    JsonFoodRepository repo{"/home/anas/personal_projects/calorie-counter-backend/json_data_base.json"};
    cc::models::Food food;
...
TEST_F(JsonFoodRepositoryTest,save_and_data_base_empty) {
    ...
    JsonFoodRepository repo_temp{"/home/anas/personal_projects/calorie-counter-backend/json_data_base_temp.json"};
    EXPECT_NO_THROW(repo_temp.save(food));
    EXPECT_EQ(repo_temp.getById_or_Barcode(food.id()).unwrap().id(), food.id());
    EXPECT_EQ(repo_temp.getById_or_Barcode(food.id()).unwrap().name(), food.name());
    EXPECT_EQ(repo_temp.getById_or_Barcode(food.id()).unwrap().brand(), food.brand());
    EXPECT_EQ(repo_temp.getById_or_Barcode(food.id()).unwrap().barcode(), food.barcode());
    EXPECT_EQ(repo_temp.getById_or_Barcode(food.id()).unwrap().source(), food.source());
    EXPECT_EQ(repo_temp.getById_or_Barcode(food.id()).unwrap().imageUrl(), food.imageUrl());
    EXPECT_EQ(repo_temp.getById_or_Barcode(food.id()).unwrap().caloriesPer100g(), food.caloriesPer100g());
    EXPECT_EQ(repo_temp.getById_or_Barcode(food.id()).value.value().totalKcal(), food.totalKcal());
    EXPECT_EQ(repo_temp.getById_or_Barcode(food.id()).unwrap().to_string(), food.to_string());
    EXPECT_EQ(repo_temp.getById_or_Barcode(food.id()).unwrap().nutrients()[0].name(), food.nutrients()[0].name());
    EXPECT_EQ(repo_temp.getById_or_Barcode(food.id()).unwrap().nutrients()[1].name(), food.nutrients()[1].name());
...
TEST_F(JsonFoodRepositoryTest,save_wrong_path_to_data_base) {
    ...
    JsonFoodRepository repo_temp{"/home/ansss/personal_projects/calorie-counter-backend/json_data_base_temp.json"};
    EXPECT_EQ(repo_temp.save(food).unwrap_error().code,cc::utils::ErrorCode::StorageError);
    EXPECT_EQ(repo_temp.getById_or_Barcode(food.id()).unwrap_error().code,cc::utils::ErrorCode::NotFound);
    }
...
TEST_F(JsonFoodRepositoryTest,getById_or_Barcode) {
    ...
    EXPECT_EQ(repo.getById_or_Barcode(food.id()).unwrap().barcode(), food.barcode());
    EXPECT_EQ(repo.getById_or_Barcode(food.id()).unwrap().source(), food.source());
    EXPECT_EQ(repo.getById_or_Barcode(food.id()).unwrap().imageUrl(), food.imageUrl());
    EXPECT_EQ(repo.getById_or_Barcode(food.id()).unwrap().caloriesPer100g(), food.caloriesPer100g());
    EXPECT_EQ(repo.getById_or_Barcode(food.id()).value.value().totalKcal(), food.totalKcal());
    EXPECT_EQ(repo.getById_or_Barcode(food.id()).unwrap().to_string(), food.to_string());
    EXPECT_EQ(repo.getById_or_Barcode(food.id()).unwrap().nutrients()[0].name(), food.nutrients()[0].name());
    EXPECT_EQ(repo.getById_or_Barcode(food.id()).unwrap().nutrients()[1].name(), food.nutrients()[1].name());
    }

TEST_F(JsonFoodRepositoryTest,getById_or_Barcode_ITEM_NOT_FOUND) {
    EXPECT_NO_THROW(repo.save(food));
    std::string wrong_id =std::format("{}wrong",food.id());
    EXPECT_EQ(repo.getById_or_Barcode(wrong_id).unwrap_error().code, cc::utils::ErrorCode::NotFound);
    }
...
TEST_F(JsonFoodRepositoryTest,list_cant_open_data_base) {
    //wrong_path
    JsonFoodRepository repo_temp{"/home/anasss/personal_projects/calorie-counter-backend/json_data_base_temp.json"};
    EXPECT_EQ(repo_temp.save(food).unwrap_error().code,cc::utils::ErrorCode::StorageError);
...
TEST_F(JsonFoodRepositoryTest,remove_wrong_path_to_data_base) {
    ...
    EXPECT_EQ(repo_temp.save(food).unwrap_error().code,cc::utils::ErrorCode::StorageError);
    EXPECT_EQ(repo_temp.list().unwrap_error().code,cc::utils::ErrorCode::NotFound);
    EXPECT_EQ(repo_temp.remove(food.id()).unwrap_error().code,cc::utils::ErrorCode::NotFound);
    }

TEST_F(JsonFoodRepositoryTest,upsert) {
    JsonFoodRepository repo_temp{"/home/anas/personal_projects/calorie-counter-backend/json_data_base_temp_debug_upsert.json"};
    ...
    EXPECT_EQ(repo_temp.getById_or_Barcode(food.id()).unwrap().id(), food.id());
    std::remove("/home/anas/personal_projects/calorie-counter-backend/json_data_base_temp_debug_upsert.json");
    }
